...
$@"        <a-form-model-item label=""{aField.Description}"" prop=""{aField.Name}"">
        ...
        {
            string nameSpace = $@"LiuLiangPeng.Entity.{_areaName}";
            string filePath = Path.Combine(_solutionPath, "LiuLiangPeng.Entity", _areaName, $"{tableName}.cs");
        ...
        {
            string content = File.ReadAllText(Path.Combine(_solutionPath, "LiuLiangPeng.Api", "BuildCodeTemplate", templateFileName));
